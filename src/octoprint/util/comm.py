...
def serialList():
	...
	if not settings().getBoolean(["serial", "useOnlyAdditionalPorts"]):
		if os.name=="nt":
			try:
	...
	if additionalPorts:
	  ...
	  baselist = list(set(baselist))	# remove possible duplicates through globs
	  if not settings().getBoolean(["serial", "useOnlyAdditionalPorts"]):
		  baselist.insert(0,"AUTO") # instead of inserting it in template
